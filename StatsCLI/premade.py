...
if __name__ == "__main__":
	...
	# add ltime and rtime by default? would remove need to do where logic
	# add in ltime limit

	if args.ltime is not None:
		ltime = convertTime(args.ltime)
		commands = [command + f"{' AND' if 'WHERE' in command else ' WHERE'} time >= {ltime}" for command in commands]

	# add in rtime limit
	if args.rtime is not None:
		rtime = convertTime(args.rtime)
		commands = [command + f"{' AND' if 'WHERE' in command else ' WHERE'} time <= {rtime}" for command in commands]
	...
	if len(args.args) > 1: # if we expect arguments from user then format them here
